...
def get_id_from_db(server,db_name):
	"""Get the minimum ID from the database"""
	...
	try:
		cursor.execute(f"SELECT MAX(id) AS min_id FROM {db_name}.tbl_site_asset;")
		print(f"SELECT MAX(id) AS min_id FROM {db_name}.tbl_site_asset;") #debugging
		result = cursor.fetchone()
		if result and 'min_id' in result:
			return result['min_id']
		else:
			print("No data found in tbl_site_asset")
			return None
	except Exception as e:
		print(f"Error fetching min_id: {e}")
		return None
	finally:
		cursor.close()
		conx.close()
...
def upload_to_database_master_F(master_file):
	...
	for column in lhs_df.columns:
		...
		if should_skip_asset(check,skip_assets):
			continue
